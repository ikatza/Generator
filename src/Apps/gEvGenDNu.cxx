// TODO DNU: Have only changed every instance of dm for dnu
// TODO DNU: Check consistency of whole file
...
\brief   A simple 'generic' GENIE DarkNeutrino event generation driver (gevgen_dnu).

         It handles:
         a) event generation for a fixed init state (DNu+A) at fixed energy, or
         ...
         Options :
           ...
           -h
              Prints-out help on using gevgen_dnu and exits.
           -n
...
double          gOptDNuEnergy;     // dark matter E, or min dark matter energy in spectrum
double          gOptDNuEnergyRange;// energy range in input spectrum
double          gOptDNuMass;       // dark matter mass
double          gOptZpCoupling;   // mediator coupling
map<int,double> gOptTgtMix;       // target mix (each with its relative weight)
double          gOptMedRatio;     // ratio of mediator to DNu mass
...
int main(int argc, char ** argv)
{
  ...
  PDGLibrary::Instance()->AddDarkMatter(gOptDNuMass,gOptMedRatio);
  if (gOptZpCoupling > 0.) {
...
void GenerateEventsAtFixedInitState(void)
{
  ...
  double Ed    = gOptDNuEnergy;
  double Md    = gOptDNuMass;
  double pd    = TMath::Sqrt(Ed*Ed - Md*Md);
  assert(pd>=0.);
  TLorentzVector dnu_p4(0.,0.,pd,Ed); // px,py,pz,E (GeV)
  ...
  while (ievent < gOptNevents) {
    ...
    EventRecord * event = evg_driver.GenerateEvent(dnu_p4);
...
void GenerateEventsUsingFluxOrTgtMix(void)
{
  ...
  while ( ievent < gOptNevents) {
     ...
     LOG("gevgen_dnu", pNOTICE) << " *** Generating event............ " << ievent;
     ...
     LOG("gevgen_dnu", pNOTICE) << "Generated Event GHEP Record: " << *event;
...
//
  ...
  if(gOptDNuEnergyRange<0) flux_driver = MonoEnergeticFluxDriver();
...
//
  flux::GMonoEnergeticFlux * flux =
              new flux::GMonoEnergeticFlux(gOptDNuEnergy, kPdgDarkMatter);
...
//
  ...
  double emin = gOptDNuEnergy;
  double emax = gOptDNuEnergy+gOptDNuEnergyRange;
  double de   = gOptDNuEnergyRange;
  ...
  if(input_is_text_file) {
    ...
    for(int ientry=0; ientry<flux_entries; ientry++) {
      ...
      while(!accept) {
        ...
        if(iter > kRjMaxIterations) {
           LOG("gevgen_dnu", pFATAL) << "Couldn't generate a flux histogram";
           exit(1);
        }
    ...
    LOG("gevgen_dnu", pNOTICE) << "Getting input flux from root file: " << fv[0];
    TFile * flux_file = new TFile(fv[0].c_str(),"read");

    LOG("gevgen_dnu", pNOTICE) << "Flux name: " << fv[1];
    TH1D * hst = (TH1D *)flux_file->Get(fv[1].c_str());
    assert(hst);

    LOG("gevgen_dnu", pNOTICE) << hst->GetEntries();
    ...
    LOG("gevgen_dnu", pNOTICE) << spectrum->GetEntries();
...
void GetCommandLineArgs(int argc, char ** argv)
{
  LOG("gevgen_dnu", pINFO) << "Parsing command line arguments";
  ...
  if ( ! parser.OptionExists("tune") ) {
    LOG("gevgen_dnu", pFATAL) << "No Dark Matter tune selected, please select one ";
    LOG("gevgen_dnu", pFATAL) << "Exiting ";
    exit( 0 ) ;
  }

  // number of events
  if( parser.OptionExists('n') ) {
    LOG("gevgen_dnu", pINFO) << "Reading number of events to generate";
    gOptNevents = parser.ArgAsInt('n');
  } else {
    LOG("gevgen_dnu", pINFO)
  ...
  if( parser.OptionExists('r') ) {
    LOG("gevgen_dnu", pINFO) << "Reading MC run number";
    gOptRunNu = parser.ArgAsLong('r');
  } else {
    LOG("gevgen_dnu", pINFO) << "Unspecified run number - Using default";
    gOptRunNu = kDefOptRunNu;
  }

  // Output file name
  if( parser.OptionExists('o') ) {
    LOG("gevgen_dnu", pINFO) << "Reading output file name";
  ...
  if( parser.OptionExists('f') ) {
    LOG("gevgen_dnu", pINFO) << "Reading flux function";
    gOptFlux = parser.ArgAsString('f');
    using_flux = true;
  }

  if(parser.OptionExists('s')) {
    LOG("gevgen_dnu", pWARN)
  ...
  if( parser.OptionExists('e') ) {
    LOG("gevgen_dnu", pINFO) << "Reading dark matter energy";
    string dnue = parser.ArgAsString('e');

    // is it just a value or a range (comma separated set of values)
    if(dnue.find(",") != string::npos) {
       // split the comma separated list
       vector<string> nurange = utils::str::Split(dnue, ",");
       assert(nurange.size() == 2);
       double emin = atof(nurange[0].c_str());
       double emax = atof(nurange[1].c_str());
       assert(emax>emin && emin>=0);
       gOptDNuEnergy      = emin;
       gOptDNuEnergyRange = emax-emin;
       if(!using_flux) {
          LOG("gevgen_dnu", pWARN)
             << "No flux was specified but an energy range was input!";
          LOG("gevgen_dnu", pWARN)
             << "Events will be generated at fixed E = " << gOptDNuEnergy << " GeV";
          gOptDNuEnergyRange = -1;
       }
    } else {
       gOptDNuEnergy       = atof(dnue.c_str());
       gOptDNuEnergyRange = -1;
    }
  } else {
    LOG("gevgen_dnu", pFATAL) << "Unspecified dark matter energy - Exiting";
    PrintSyntax();
    exit(1);
  }
  ...
  if( parser.OptionExists('m') ) {
    LOG("gevgen_dnu", pINFO) << "Reading dark matter mass";
    gOptDNuMass = parser.ArgAsDouble('m');
  } else {
    LOG("gevgen_dnu", pFATAL) << "Unspecified dark matter mass - Exiting";
    PrintSyntax();
    exit(1);
  }
  ...
  if( parser.OptionExists('g') ) {
    LOG("gevgen_dnu", pINFO) << "Reading mediator coupling";
    gOptZpCoupling = parser.ArgAsDouble('g');
  } else {
    LOG("gevgen_dnu", pINFO) << "Unspecified mediator coupling - Using value from config file";
    gOptZpCoupling = -1.;
  }
  ...
  if( parser.OptionExists('t') ) {
    LOG("gevgen_dnu", pINFO) << "Reading target mix";
    ...
    LOG("gevgen_dnu", pFATAL) << "Unspecified target PDG code - Exiting";
    PrintSyntax();
    exit(1);
  }
  ...
  if( parser.OptionExists('z') ) {
    LOG("gevgen_dnu", pINFO) << "Reading mediator mass ratio";
    gOptMedRatio = parser.ArgAsDouble('z');
  } else {
    LOG("gevgen_dnu", pINFO) << "Unspecified mediator mass ratio - Using default";
    gOptMedRatio = 0.5;
  }
  ...
  if( parser.OptionExists("seed") ) {
    LOG("gevgen_dnu", pINFO) << "Reading random number seed";
    gOptRanSeed = parser.ArgAsLong("seed");
  } else {
    LOG("gevgen_dnu", pINFO) << "Unspecified random number seed - Using default";
    gOptRanSeed = -1;
  }

  // input cross-section file
  if( parser.OptionExists("cross-sections") ) {
    LOG("gevgen_dnu", pINFO) << "Reading cross-section file";
    gOptInpXSecFile = parser.ArgAsString("cross-sections");
  } else {
    LOG("gevgen_dnu", pINFO) << "Unspecified cross-section file";
    gOptInpXSecFile = "";
  }
  ...
  LOG("gevgen_dnu", pNOTICE)
     << "\n"
     << utils::print::PrintFramedMesg("gevgen_dnu job configuration");
  LOG("gevgen_dnu", pNOTICE)
  ...
  if(gOptDNuEnergyRange>0) {
     LOG("gevgen_dnu", pNOTICE)
        << "Dark matter energy: ["
        << gOptDNuEnergy << ", " << gOptDNuEnergy+gOptDNuEnergyRange << "]";
  } else {
     LOG("gevgen_dnu", pNOTICE)
        << "Dark matter energy: " << gOptDNuEnergy;
  }
  LOG("gevgen_dnu", pNOTICE)
      << "Dark matter mass: " << gOptDNuMass;
  LOG("gevgen_dnu", pNOTICE)
  ...
  LOG("gevgen_dnu", pNOTICE) << *RunOpt::Instance();
...
bool CheckUnitarityLimit(void)
{
  ...
  double Mzp  = gOptMedRatio * gOptDNuMass;
  double Mzp2 = Mzp*Mzp;
  // The leading, forward-dominated piece is the same for both DNu models
  double xsec_est = gzp4 / (4. * kPi * Mzp2);
  double ml   = gOptDNuMass;
  double ml2  = ml*ml;
