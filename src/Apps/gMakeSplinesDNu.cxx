    ...
    for (vector<double>::iterator ratio = gOptMedRatios.begin(); ratio != gOptMedRatios.end(); ++ratio) {
      for (vector<double>::iterator coup = gOptZpCouplings.begin(); coup != gOptZpCouplings.end(); ++coup) {
        ...
        if(!targets || targets->size() == 0 ) {
          LOG("gmkspl_dnu", pFATAL) << "Empty target PDG code list";
          PrintSyntax();
          exit(3);
        }

        LOG("gmkspl_dnu", pINFO) << "Targets: "   << *targets;
...
void GetCommandLineArgs(int argc, char ** argv)
{
  LOG("gmkspl_dnu", pINFO) << "Parsing command line arguments";
  ...
  {
    LOG("gmkspl_dnu", pINFO) << "Reading output filename";
    if( parser.OptionExists('o') ) {
    ...
    LOG("gmkspl_dnu", pINFO) << "Unspecified filename - Using default";
    gOptOutXSecFile = "xsec_splines.xml";
  }

  // number of knots
  if( parser.OptionExists('n') ) {
    LOG("gmkspl_dnu", pINFO) << "Reading number of knots/spline";
    gOptNKnots = parser.ArgAsInt('n');
  } else {
    LOG("gmkspl_dnu", pINFO)
  ...
  if( parser.OptionExists('e') ) {
    LOG("gmkspl_dnu", pINFO) << "Reading maximum spline energy";
  ...
  if( parser.OptionExists('m') ) {
    LOG("gmkspl_dnu", pINFO) << "Reading dark matter mass";
  ...
  if( parser.OptionExists('z') ) {
    LOG("gmkspl_dnu", pINFO) << "Reading mediator mass ratios";
  ...
  if( parser.OptionExists("no-copy") ) {
    LOG("gmkspl_dnu", pINFO) << "Not copying input splines to output";
    gOptNoCopy = true;
  }

  // get the mediator coupling
  if( parser.OptionExists('g') ) {
    LOG("gmkspl_dnu", pINFO) << "Reading mediator couplings";
  ...
  if( parser.OptionExists('t') ) {
    LOG("gmkspl_dnu", pINFO) << "Reading target nuclei PDG codes";
    gOptTgtPdgCodeList = parser.ArgAsString('t');
  } else {
    LOG("gmkspl_dnu", pINFO) << "No code list specified from the command line";
    tgt_cmd = false;
  }

  bool tgt_geom = true;
  if( parser.OptionExists('f') ) {
    LOG("gmkspl_dnu", pINFO) << "Reading ROOT geometry filename";
    gOptGeomFilename = parser.ArgAsString('f');
  } else {
    LOG("gmkspl_dnu", pINFO) << "No geometry file was specified";
    tgt_cmd = false;
  }
  ...
  if( parser.OptionExists("seed") ) {
    LOG("gmkspl_dnu", pINFO) << "Reading random number seed";
    gOptRanSeed = parser.ArgAsLong("seed");
  } else {
    LOG("gmkspl_dnu", pINFO) << "Unspecified random number seed - Using default";
    gOptRanSeed = -1;
  }

  // input cross-section file
  if( parser.OptionExists("input-cross-sections") ) {
    LOG("gmkspl_dnu", pINFO) << "Reading cross-section file";
    gOptInpXSecFile = parser.ArgAsString("input-cross-sections");
  } else {
    LOG("gmkspl_dnu", pINFO) << "Unspecified input cross-section file";
    gOptInpXSecFile = "";
  }
  ...
  LOG("gmkspl_dnu", pNOTICE)
     << "\n"
     << utils::print::PrintFramedMesg("gmkspl_dnu job configuration")
  ...
  LOG("gmkspl_dnu", pNOTICE) << *RunOpt::Instance();
}
//____________________________________________________________________________
void PrintSyntax(void)
{
...
PDGCodeList * GetTargetCodes(void)
{
  ...
  if (from_geom_file) {
     ...
     LOG("gmkspl_dnu", pINFO) << "Creating/configuring a ROOT geom. driver";
