...
\brief   A GENIE event generation driver `customized' for the FNAL neutrino
         ...
         *** Options :
           ...
           -h
              Prints out the gevgen_lardnu syntax and exits.
           -r
...
int main(int argc, char ** argv)
{
  ...
  LOG("gevgen_lardnu", pNOTICE) << "Done!";
...
void GetCommandLineArgs(int argc, char ** argv)
{
  LOG("gevgen_lardnu", pINFO) << "Parsing command line arguments";
  ...
  if ( parser.OptionExists('r') ) {
    LOG("gevgen_lardnu", pDEBUG) << "Reading MC run number";
    gOptRunNu = parser.ArgAsLong('r');
  } else {
    LOG("gevgen_lardnu", pDEBUG)
  ...
  if( parser.OptionExists('g') ) {
    LOG("gevgen_lardnu", pDEBUG) << "Getting input geometry";
  ...
  if( parser.OptionExists('M') ) {
    LOG("gevgen_lardnu", pINFO) << "Reading dark matter mass";
    gOptDMMass = parser.ArgAsDouble('M');
  } else {
    LOG("gevgen_lardnu", pFATAL) << "Unspecified dark matter mass - Exiting";
    PrintSyntax();
    exit(1);
  } // -M
  ...
  if( parser.OptionExists('c') ) {
    LOG("gevgen_lardnu", pINFO) << "Reading mediator coupling";
    gOptZpCoupling = parser.ArgAsDouble('c');
  } else {
    LOG("gevgen_lardnu", pINFO) << "Unspecified mediator coupling - Using value from config file";
    gOptZpCoupling = -1.;
  } // -c

  // mediator mass ratio
  if( parser.OptionExists('v') ) {
    LOG("gevgen_lardnu", pINFO) << "Reading mediator mass ratio";
    gOptMedRatio = parser.ArgAsDouble('v');
  } else {
    LOG("gevgen_lardnu", pINFO) << "Unspecified mediator mass ratio - Using default";
    gOptMedRatio = 0.5;
  } // -v
  ...
  if(gOptUsingRootGeom) {
     ...
     if( parser.OptionExists('L') ) {
        ...
        LOG("gevgen_lardnu", pDEBUG) << "Using default geometry length units";
     ...
     if( parser.OptionExists('D') ) {
        ...
        LOG("gevgen_lardnu", pDEBUG) << "Using default geometry density units";
     ...
     if( parser.OptionExists('t') ) {
        LOG("gevgen_lardnu", pDEBUG) << "Checking for input volume name";
        gOptRootGeomTopVol = parser.ArgAsString('t');
     } else {
        LOG("gevgen_lardnu", pDEBUG) << "Using the <master volume>";
     } // -t
     ...
     if( parser.OptionExists('F') ) {
       LOG("gevgen_lardnu", pDEBUG) << "Using Fiducial cut?";
       gOptFidCut = parser.ArgAsString('F');
     } else {
       LOG("gevgen_lardnu", pDEBUG) << "No fiducial volume cut";
       gOptFidCut = "";
     } //-F

     // how to scan the geometry (if relevant)
     if( parser.OptionExists('S') ) {
         LOG("gevgen_lardnu", pDEBUG)  << "Reading requested geom scan count";
         gOptNScan = parser.ArgAsInt('S');
     } else {
         LOG("gevgen_lardnu", pDEBUG) << "No geom scan count was requested";
         gOptNScan = 0;
     } //-S

     // z for flux rays to start
     if( parser.OptionExists('z') ) {
         LOG("gevgen_lardnu", pDEBUG)  << "Reading requested zmin";
         gOptZmin = parser.ArgAsDouble('z');
     } else {
         LOG("gevgen_lardnu", pDEBUG) << "No zmin was requested";
         gOptZmin = -2.0e30; // < -1.0e30 ==> leave it on flux window
     } //-z

     // debug flags
     if ( parser.OptionExists('d') ) {
         LOG("gevgen_lardnu", pDEBUG) << "Reading debug flag value";
         gOptDebug = parser.ArgAsInt('d');
       } else {
         LOG("gevgen_lardnu", pDEBUG) << "Unspecified debug flags - Using default";
         gOptDebug = 0;
       } //-d
  ...
  //
  if ( parser.OptionExists('f') ) {
    LOG("gevgen_lardnu", pDEBUG) << "Getting input flux";
    gOptFluxFile = parser.ArgAsString('f');
  } else {
    LOG("gevgen_lardnu", pFATAL) << "No flux info was specified - Exiting";
    PrintSyntax();
    exit(1);
  }
  ...
  if( parser.OptionExists('o') ) {
    LOG("gevgen_lardnu", pDEBUG) << "Reading the event filename prefix";
  ...
  if( parser.OptionExists("seed") ) {
    LOG("gevgen_lardnu", pINFO) << "Reading random number seed";
    gOptRanSeed = parser.ArgAsLong("seed");
  } else {
    LOG("gevgen_lardnu", pINFO) << "Unspecified random number seed - Using default";
    gOptRanSeed = -1;
  }

  // input cross-section file
  if( parser.OptionExists("cross-sections") ) {
    LOG("gevgen_lardnu", pINFO) << "Reading cross-section file";
    gOptInpXSecFile = parser.ArgAsString("cross-sections");
  } else {
    LOG("gevgen_lardnu", pINFO) << "Unspecified cross-section file";
    gOptInpXSecFile = "";
  }
  ...
  LOG("gevgen_lardnu", pNOTICE) << *RunOpt::Instance();
}
//____________________________________________________________________________
void PrintSyntax(void)
{
...
void CreateFidSelection (string fidcut, GeomAnalyzerI* geom_driver)
{
  ...
  LOG("gevgen_lardnu", pNOTICE) << "-F " << fidcut;
  ...
  if        ( stype.find("zcyl")   != string::npos ) {
    ...
    if ( nvals < 5 )
      LOG("gevgen_lardnu", pFATAL) << "MakeZCylinder needs 5 values, not " << nvals
                                << " fidcut=\"" << fidcut << "\"";
    ...
    if ( nvals < 6 )
      LOG("gevgen_lardnu", pFATAL) << "MakeBox needs 6 values, not " << nvals
                                << " fidcut=\"" << fidcut << "\"";
    ...
    if ( nvals < 7 )
      LOG("gevgen_lardnu", pFATAL) << "MakeZPolygon needs 7 values, not " << nvals
                                << " fidcut=\"" << fidcut << "\"";
    int nfaces = (int)vals[0];
    if ( nfaces < 3 )
      LOG("gevgen_lardnu", pFATAL) << "MakeZPolygon needs nfaces>=3, not " << nfaces
                                << " fidcut=\"" << fidcut << "\"";
    ...
    if ( nvals < 4 )
      LOG("gevgen_lardnu", pFATAL) << "MakeZSphere needs 4 values, not " << nvals
                                << " fidcut=\"" << fidcut << "\"";
  ...
  if ( master  ) {
    ...
    LOG("gevgen_lardnu", pNOTICE) << "Convert fiducial volume from master to topvol coords";
  }
  if ( reverse ) {
    fidsel->SetReverseFiducial(true);
    LOG("gevgen_lardnu", pNOTICE) << "Reverse sense of fiducial volume cut";
  }
