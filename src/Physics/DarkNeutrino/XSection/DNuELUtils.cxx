...
double genie::utils::EnergyDeltaFunctionSolutionDNuEL(
  const genie::Interaction& inter)
{
  ...
  // Compute the factor in the kPSDNuELEvGen phase space that arises from
...
double genie::utils::ComputeFullDNuELPXSec(genie::Interaction* interaction,
  const genie::NuclearModelI* nucl_model, const genie::XSecAlgorithmI* xsec_model,
  double cos_theta_0, double phi_0, double& Eb,
  genie::DNuELEvGen_BindingMode_t hitNucleonBindingMode, double min_angle_EM,
  bool bind_nucleon)
{
  ...
  double xsec = xsec_model->XSec(interaction, genie::kPSDNuELEvGen);

  return xsec;
}

genie::DNuELEvGen_BindingMode_t genie::utils::StringToDNuELBindingMode(
  const std::string& binding_mode)
{
  ...
  if ( binding_mode == "UseGroundStateRemnant" ) {
    ...
    LOG("DNuELEvent", pFATAL) << "Unrecognized setting \"" << binding_mode
      << "\" requested in genie::utils::StringToDNuELBindingMode()";
    gAbortingInErr = true;
    std::exit(1);
  }
}
...
void genie::utils::BindHitNucleon(genie::Interaction& interaction,
  ...
  genie::DNuELEvGen_BindingMode_t hitNucleonBindingMode)
{
