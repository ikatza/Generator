...
#include "Physics/DarkNeutrino/XSection/QPMDNuDISPXSec.h"
...
//____________________________________________________________________________
QPMDNuDISPXSec::QPMDNuDISPXSec() :
XSecAlgorithmI("genie::QPMDNuDISPXSec")
{
  fInInitPhase = true;
}
//____________________________________________________________________________
QPMDNuDISPXSec::QPMDNuDISPXSec(string config) :
XSecAlgorithmI("genie::QPMDNuDISPXSec", config)
{
  fInInitPhase = true;
}
//____________________________________________________________________________
     ...
     const Interaction * interaction, KinePhaseSpace_t kps) const
{
  ...
  LOG("DNuDISPXSec", pDEBUG)  
   << "Computing d2xsec/dxdy @ E = " << E << ", x = " << x << ", y = " << y;
#endif

  // One of the xsec terms changes sign for antineutrinos @ DNuDIS/CC
  ...
  // Calculate the DNuDIS structure functions
  fDISSF.Calculate(interaction); 

#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
  LOG("DNuDISPXSec", pDEBUG) << fDISSF;
#endif
...
//____________________________________________________________________________
double QPMDNuDISPXSec::Integral(const Interaction * interaction) const
{
  double xsec = fXSecIntegrator->Integrate(this,interaction);
  return xsec;
}
//____________________________________________________________________________
bool QPMDNuDISPXSec::ValidProcess(const Interaction * interaction) const
{
...
//____________________________________________________________________________
void QPMDNuDISPXSec::Configure(const Registry & config)
{
  Algorithm::Configure(config);
  this->LoadConfig();
}
//____________________________________________________________________________
void QPMDNuDISPXSec::Configure(string config)
{
  Algorithm::Configure(config);

  Registry r( "QPMDNuDISPXSec_specific", false ) ;
...
//____________________________________________________________________________
void QPMDNuDISPXSec::LoadConfig(void)
{
  ...
  // Caching the reduction factors used in the DNuDIS/RES joing scheme?
  ...
  if(!fInInitPhase) {
     ...
     string keysubstr = this->Id().Key() + "/DNuDIS-RES-Join";
