...
#include "Physics/DarkNeutrino/XSection/DNuDISXSec.h"
...
//____________________________________________________________________________
DNuDISXSec::DNuDISXSec() :
XSecIntegratorI("genie::DNuDISXSec")
{

}
//____________________________________________________________________________
DNuDISXSec::DNuDISXSec(string config) :
XSecIntegratorI("genie::DNuDISXSec", config)
{
                 ...
                 const XSecAlgorithmI * model, const Interaction * in) const
{
  ...
  if(!kps.IsAboveThreshold()) {
     LOG("DNuDISXSec", pDEBUG)  << "*** Below energy threshold";
     return 0;
  }
  ...
  if(precalc_bare_xsec) {
     ...
     LOG("DNuDISXSec", pINFO) << "Finding cache branch with key: " << key;
...
//____________________________________________________________________________
void DNuDISXSec::Configure(const Registry & config)
{
  Algorithm::Configure(config);
  this->LoadConfig();
}
//____________________________________________________________________________
void DNuDISXSec::Configure(string config)
{
  Algorithm::Configure(config);
  this->LoadConfig();
}
//____________________________________________________________________________
void DNuDISXSec::LoadConfig(void)
{
          ...
          const XSecAlgorithmI * model, const Interaction * interaction) const
{
  ...
  CacheBranchFx * cache_branch =
           dynamic_cast<CacheBranchFx *> (cache->FindCacheBranch(key));
  assert(!cache_branch);
  cache_branch = new CacheBranchFx("DNuDIS XSec");
  ...
  for(int ie=0; ie<nknots; ie++) {
    LOG("DNuDISXSec", pDEBUG) << "Dealing with knot " << ie << " out of " << nknots;
    double Ed = E[ie];
    ...
    LOG("DNuDISXSec", pNOTICE)  
       << "Caching: XSec[DNuDIS] (E = " << Ed << " GeV) = " 
...
//____________________________________________________________________________
string DNuDISXSec::CacheBranchName(
          const XSecAlgorithmI * model, const Interaction * interaction) const
{
