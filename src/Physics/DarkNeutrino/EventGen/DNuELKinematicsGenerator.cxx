...
#include "Physics/DarkNeutrino/EventGen/DNuELKinematicsGenerator.h"
...
DNuELKinematicsGenerator::DNuELKinematicsGenerator() :
KineGeneratorWithCache("genie::DNuELKinematicsGenerator")
{

}
//___________________________________________________________________________
DNuELKinematicsGenerator::DNuELKinematicsGenerator(string config) :
KineGeneratorWithCache("genie::DNuELKinematicsGenerator", config)
{
...
//___________________________________________________________________________
void DNuELKinematicsGenerator::ProcessEventRecord(GHepRecord * evrec) const
{
  if(fGenerateUniformly) {
    LOG("DNuELKinematics", pNOTICE)
  ...
  //   the next attempted event will be forced to DNu again.
  ...
  if(Q2.max <=0 || Q2.min>=Q2.max) {
     LOG("DNuELKinematics", pWARN) << "No available phase space";
  ...
  LOG("DNuELKinematics",pNOTICE) << "Setting max XSec";
  double xsec_max = (fGenerateUniformly) ? -1 : this->MaxXSec(evrec);
  LOG("DNuELKinematics",pNOTICE) << "Set max XSec to " << xsec_max;
...
*/
     LOG("DNuELKinematics",pNOTICE) << "Attempting to sample";
     gQ2 = Q2min + (Q2max-Q2min) * rnd->RndKine().Rndm();
     interaction->KinePtr()->SetQ2(gQ2);
     LOG("DNuELKinematics", pINFO) << "Trying: Q^2 = " << gQ2;
     ...
     if(accept) {
        LOG("DNuELKinematics", pINFO) << "Selected: Q^2 = " << gQ2;
        ...
        // For DNu/Charm events it is set to be equal to the on-shell mass of
        ...
        LOG("DNuELKinematics", pNOTICE) << "Selected: W = "<< gW;
        ...
        if(fGenerateUniformly) {
          ...
          LOG("DNuELKinematics", pNOTICE)  << "Kinematics wght = "<< wght;

          // apply computed weight to the current event weight
          wght *= evrec->Weight();
          LOG("DNuELKinematics", pNOTICE) << "Current event wght = " << wght;
...
//___________________________________________________________________________
void DNuELKinematicsGenerator::SpectralFuncExperimentalCode(
...
//  double xsec_max = (fGenerateUniformly) ? -1 : this->MaxXSec(evrec);
  ...
  LOG("DNuELKinematics", pNOTICE) << "E = " << E << ", M = "<< Mn;
  ...
  while(1) {
     ...
     //-- Generate a Q2 value within the allowed phase space
     gQ2 = Q2min + (Q2max-Q2min) * rnd->RndKine().Rndm();
     LOG("DNuELKinematics", pNOTICE) << "Trying: Q^2 = " << gQ2;
     ...
     LOG("DNuELKinematics", pNOTICE) << "W = "<< gW;
     LOG("DNuELKinematics", pNOTICE) << "x = "<< gx;
     LOG("DNuELKinematics", pNOTICE) << "y = "<< gy;

     // v
     double gv  = gy * E;
     double gv2 = gv*gv;

     LOG("DNuELKinematics", pNOTICE) << "v = "<< gv;

     // v -> v~
     double gvtilde  = gv + Mn - Eb - TMath::Sqrt(Mn*Mn+pxn*pxn+pyn*pyn+pzn*pzn);
     double gvtilde2 = gvtilde*gvtilde;

     LOG("DNuELKinematics", pNOTICE) << "v~ = "<< gvtilde;

     // Q~^2
     double gQ2tilde = gQ2 - gv2 + gvtilde2;

     LOG("DNuELKinematics", pNOTICE) << "Q~^2 = "<< gQ2tilde;
...
//     }
     ...
     //-- If the generated kinematics are accepted, finish-up module's job
     if(accept) {
        LOG("DNuELKinematics", pNOTICE) << "Selected: Q^2 = " << gQ2;
...
//___________________________________________________________________________
void DNuELKinematicsGenerator::Configure(const Registry & config)
{
  Algorithm::Configure(config);
  this->LoadConfig();
}
//____________________________________________________________________________
void DNuELKinematicsGenerator::Configure(string config)
{
  Algorithm::Configure(config);
  this->LoadConfig();
}
//____________________________________________________________________________
void DNuELKinematicsGenerator::LoadConfig(void)
{
...
//____________________________________________________________________________
double DNuELKinematicsGenerator::ComputeMaxXSec(
...
// safety factor. But it needs to be fast - do not use a very small dQ2 step.
  ...
  LOG("DNuELKinematics", pDEBUG) << "Range of Q^2: " << rQ2.min << " to " << rQ2.max;
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
     LOG("DNuELKinematics", pDEBUG)  << "xsec(Q2= " << Q2 << ") = " << xsec;
#endif
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
  SLOG("DNuELKinematics", pDEBUG) << interaction->AsString();
  SLOG("DNuELKinematics", pDEBUG) << "Max xsec in phase space = " << max_xsec;
  SLOG("DNuELKinematics", pDEBUG) << "Computed using alg = " << *fXSecModel;
#endif
