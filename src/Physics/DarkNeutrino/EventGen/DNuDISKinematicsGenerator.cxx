...
#include "Physics/DarkNeutrino/EventGen/DNuDISKinematicsGenerator.h"
...
DNuDISKinematicsGenerator::DNuDISKinematicsGenerator() :
KineGeneratorWithCache("genie::DNuDISKinematicsGenerator")
{

}
//___________________________________________________________________________
DNuDISKinematicsGenerator::DNuDISKinematicsGenerator(string config) :
KineGeneratorWithCache("genie::DNuDISKinematicsGenerator", config)
{
...
//___________________________________________________________________________
void DNuDISKinematicsGenerator::ProcessEventRecord(GHepRecord * evrec) const
{
  if(fGenerateUniformly) {
    LOG("DNuDISKinematics", pNOTICE)
  ...
  if(W.max <=0 || W.min>=W.max) {
     LOG("DNuDISKinematics", pWARN) << "No available phase space";
  ...
  LOG("DNuDISKinematics", pNOTICE) << "x: [" << xl.min << ", " << xl.max << "]";
  LOG("DNuDISKinematics", pNOTICE) << "y: [" << yl.min << ", " << yl.max << "]";
	...
	double J = 1;
     ...
     if(accept) {
         ...
         if(fGenerateUniformly) {
            ...
            LOG("DNuDISKinematics", pNOTICE)  << "Kinematics wght = "<< wght;

            // apply computed weight to the current event weight
            wght *= evrec->Weight();
            LOG("DNuDISKinematics", pNOTICE) << "Current event wght = " << wght;
...
//___________________________________________________________________________
void DNuDISKinematicsGenerator::Configure(const Registry & config)
{
  Algorithm::Configure(config);
  this->LoadConfig();
}
//____________________________________________________________________________
void DNuDISKinematicsGenerator::Configure(string config)
{
  Algorithm::Configure(config);
  this->LoadConfig();
}
//____________________________________________________________________________
void DNuDISKinematicsGenerator::LoadConfig(void)
{
...
//____________________________________________________________________________
double DNuDISKinematicsGenerator::ComputeMaxXSec(
...
// safety factor. But this needs to be fast - do not use a very fine grid.
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
  LOG("DNuDISKinematics", pDEBUG)<< "Computing max xsec in allowed phase space";
#endif
  double max_xsec = 0.0;
  ...
  // Look over full region or risk not finding valid phase space
  double xmin = TMath::Max(xl.min, 5E-3);
  double xmax = xl.max;
  double ymin = TMath::Max(yl.min, 2E-3);
  double ymax = yl.max;
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
     LOG("DNuDISKinematics", pDEBUG) << "y = " << gy;
#endif
     double xseclast_x = -1;
     bool increasing_x;
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
  SLOG("DNuDISKinematics", pDEBUG) << interaction->AsString();
  SLOG("DNuDISKinematics", pDEBUG) << "Max xsec in phase space = " << max_xsec;
  SLOG("DNuDISKinematics", pDEBUG) << "Computed using alg = " << *fXSecModel;
#endif
