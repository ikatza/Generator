...
#include "Physics/DarkNeutrino/EventGen/DNuEKinematicsGenerator.h"
...
DNuEKinematicsGenerator::DNuEKinematicsGenerator() :
KineGeneratorWithCache("genie::DNuEKinematicsGenerator")
{

}
//___________________________________________________________________________
DNuEKinematicsGenerator::DNuEKinematicsGenerator(string config) :
KineGeneratorWithCache("genie::DNuEKinematicsGenerator", config)
{
...
//___________________________________________________________________________
void DNuEKinematicsGenerator::ProcessEventRecord(GHepRecord * evrec) const
{
  if(fGenerateUniformly) {
    LOG("DNuEKinematics", pNOTICE)
  ...
  while(1) {
     ...
     LOG("DNuEKinematics", pINFO) << "Trying: y = " << y;
     ...
     if(!fGenerateUniformly) {
        ...
        LOG("DNuEKinematics", pDEBUG) << "xsec= "<< xsec<< ", J= 1, Rnd= "<< t;
     ...
     if(accept) {
        LOG("DNuEKinematics", pINFO) << "Selected: y = " << y;
        ...
        // wght = (phase space volume)*(differential xsec)/(event total xsec)
         if(fGenerateUniformly) {
           ...
           LOG("DNuEKinematics", pNOTICE)  << "Kinematics wght = "<< wght;

           // apply computed weight to the current event weight
           wght *= evrec->Weight();
           LOG("DNuEKinematics", pNOTICE) << "Current event wght = " << wght;
...
//___________________________________________________________________________
double DNuEKinematicsGenerator::ComputeMaxXSec(
...
//bool   increasing;
  ...
  for(int i=0; i<N; i++) {
    ...
    SLOG("DNuEKinematics", pDEBUG) << "xsec(y = " << y << ") = " << xsec;
...
*/
  ...
  SLOG("DNuEKinematics", pDEBUG) << interaction->AsString();
  SLOG("DNuEKinematics", pDEBUG) << "Max xsec in phase space = " << max_xsec;
  SLOG("DNuEKinematics", pDEBUG) << "Computed using alg = " << *fXSecModel;

  return max_xsec;
}
//___________________________________________________________________________
double DNuEKinematicsGenerator::Energy(const Interaction * interaction) const
{
...
//___________________________________________________________________________
void DNuEKinematicsGenerator::Configure(const Registry & config)
{
  Algorithm::Configure(config);
  this->LoadConfig();
}
//____________________________________________________________________________
void DNuEKinematicsGenerator::Configure(string config)
{
  Algorithm::Configure(config);
  this->LoadConfig();
}
//____________________________________________________________________________
void DNuEKinematicsGenerator::LoadConfig(void)
{
