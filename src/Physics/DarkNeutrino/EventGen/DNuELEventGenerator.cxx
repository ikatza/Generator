...
#include "Physics/DarkNeutrino/EventGen/DNuELEventGenerator.h"
...
DNuELEventGenerator::DNuELEventGenerator() :
    KineGeneratorWithCache("genie::DNuELEventGenerator")
{

}
//___________________________________________________________________________
DNuELEventGenerator::DNuELEventGenerator(string config) :
    KineGeneratorWithCache("genie::DNuELEventGenerator", config)
{
...
//___________________________________________________________________________
void DNuELEventGenerator::ProcessEventRecord(GHepRecord * evrec) const
{
    LOG("DNuELEvent", pDEBUG) << "Generating QE event kinematics...";
    ...
    if(interaction->InitState().Tgt().IsNucleus()) {
        ...
        if(! evrec->HitNucleon()) {
            LOG("DNuELEvent", pFATAL) << "No hit nucleon was set";
            gAbortingInErr = true;
            exit(1);
        }
    } // is nuclear target
    ...
    while (1) {
        ...
        iter++;
        LOG("DNuELEvent", pINFO) << "Attempt #: " << iter;
        if(iter > kRjMaxIterations) {
            LOG("DNuELEvent", pWARN)
        ...
        // Pick a direction
        // NOTE: In the kPSDNuELEvGen phase space used by this generator,
        ...
        // Set the "bind_nucleon" flag to false in this call to ComputeFullDNuELPXSec
        // since we've already done that above
        LOG("DNuELEvent", pDEBUG) << "cth0 = " << costheta << ", phi0 = " << phi;
        double xsec = genie::utils::ComputeFullDNuELPXSec(interaction, fNuclModel,
        ...
        if(accept) {
            ...
            LOG("DNuELEvent", pINFO) << "*Selected* Q^2 = " << gQ2 << " GeV^2";
            ...
            LOG("DNuELKinematics", pNOTICE) << "E = " << E << ", M = "<< M;

            // The hadronic inv. mass is equal to the recoil nucleon on-shell mass.
            // For DNuEL/Charm events it is set to be equal to the on-shell mass of
            ...
            LOG("DNuELEvent", pNOTICE) << "Selected: W = "<< gW;
            ...
            evrec->SetDiffXSec(xsec, kPSDNuELEvGen);
            ...
            LOG("DNuELEvent",pNOTICE) << "pn: " << p4ptr.X() << ", "
            ...
            LOG("DNuELEvent", pDEBUG) << "Reject current throw...";
        }

    } // iterations - while(1) loop
    LOG("DNuELEvent", pINFO) << "Done generating QE event kinematics!";
}
//___________________________________________________________________________
void DNuELEventGenerator::AddTargetNucleusRemnant(GHepRecord * evrec) const
{
    // add the remnant nuclear target at the GHEP record

    LOG("DNuELEvent", pINFO) << "Adding final state nucleus";
    ...
    LOG("DNuELEvent", pINFO) << "Done";
    LOG("DNuELEvent", pINFO) << *evrec;
}
//___________________________________________________________________________
void DNuELEventGenerator::Configure(const Registry & config)
{
    Algorithm::Configure(config);
    this->LoadConfig();
}
//____________________________________________________________________________
void DNuELEventGenerator::Configure(string config)
{
    Algorithm::Configure(config);
    this->LoadConfig();
}
//____________________________________________________________________________
void DNuELEventGenerator::LoadConfig(void)
{
    ...
    fHitNucleonBindingMode = genie::utils::StringToDNuELBindingMode( binding_mode );

    GetParamDef( "MaxXSecNucleonThrows", fMaxXSecNucleonThrows, 800 );
}
//____________________________________________________________________________
double DNuELEventGenerator::ComputeMaxXSec(const Interaction * in) const
{
    ...
    LOG("DNuELEvent", pINFO) << "Computing maximum cross section to throw against";
...

    {
        ...
        for (int ilayer = 0 ; ilayer < max_n_layers ; ilayer++) {
          ...
          for (int itheta = 0; itheta < N_theta; itheta++){
              ...
              for (int iphi = 0; iphi < N_phi; iphi++) { // Scan around phi
                  ...
                  double xs = genie::utils::ComputeFullDNuELPXSec(interaction,
        ...
        if (this_nuc_xsec_max > xsec_max) {
            ...
            LOG("DNuELEvent", pINFO) << "best estimate for xsec_max = " << xsec_max;
        }
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
    SLOG("DNuELEvent", pDEBUG) << interaction->AsString();
    SLOG("DNuELEvent", pDEBUG) << "Max xsec in phase space = " << max_xsec;
    SLOG("DNuELEvent", pDEBUG) << "Computed using alg = " << *fXSecModel;
#endif

    LOG("DNuELEvent", pINFO) << "Computed maximum cross section to throw against - value is " << xsec_max;
    return xsec_max;
}
